"""Argos Translate offline EN→Indic backend (process-level caches)."""
...
SUPPORTED_TARGETS = frozenset({"hi"})
...
class ArgosEngine:
    ...
    def translate(self, text: str, *, target_lang: str) -> str:
        ...
        if code not in SUPPORTED_TARGETS:
            raise TranslationFailedError(
                f"Unsupported translation target: {target_lang!r}. Use hi.",
...
def get_argos_engine(*, models_dir: Path | None = None) -> ArgosEngine:
    """Return the process-wide Argos engine (loaded once)."""
    global _engine
    if _engine is not None:
        return _engine
    with _lock:
        if _engine is None:
            _engine = ArgosEngine(models_dir=models_dir)
        return _engine


def reset_argos_engine() -> None:
    """Clear the cached engine (tests only)."""
    global _engine
    with _lock:
        _engine = None
